...
ticketContainer
  .bind<ITicketController<Ticket>>(ITYPES.Controller)
  .to(TicketController);
ticketContainer
  .bind<ITicketRepository<Ticket>>(ITYPES.Repository)
  .to(TicketRepository);
ticketContainer.bind(ITYPES.Datasource).toConstantValue(AppDataSource);

//Import
ticketContainer
  .bind<ISeatFlightService<SeatFlight>>(SERVICE_TYPES.SeatFlight)
  .toConstantValue(seatFlightService);
ticketContainer
  .bind<IBookingService<Booking>>(SERVICE_TYPES.Booking)
  .toConstantValue(bookingService);

const ticketController = ticketContainer.get<ITicketController<Ticket>>(
  ITYPES.Controller
);
const ticketService = ticketContainer.get<ITicketService<Ticket>>(
  ITYPES.Service
);
