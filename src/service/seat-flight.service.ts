...
@injectable()
export class SeatFlightService extends BaseService implements ISeatFlightService<any>{
    ...
    constructor(
        ...
        @inject(REPOSITORY_TYPES.TicketClass) ticketClassRepository: ITicketClassRepository<TicketClass>
    ) {
        super(repository);
    ...
    async defaultGenerateSeatForAirplane(flightId: string): Promise<any> {
        try {
            ...
            if (!defaul_class) {
                throw new BaseError(StatusCodes.INTERNAL_SERVER_ERROR, 'fail', 'Cannot find default class');
            }
            ...
            for (let i = 0; i < allSeatsId.length; i++){
                ...
                await this.repository._create({data: {seatId, flightId, class: defaul_class.className}});
            }
